...
| `prompt_rates` | What the **prompt** decay curve is fitted to, partial by partial: both components of the envelope fit, the fast one's own T60 and its share of the strike amplitude, the envelope hop that had to resolve it, and each of the curve's two guards' verdicts — at the survey's per-note geometry or at `piano-tuner estimate`'s, because the defect it convicted only exists at the first. The instrument behind `upright-parlour`'s G5, whose vl1 seventh partial decayed 60 dB in 6.6 microseconds against a 5.3 ms hop and wrote `sigma1` 8763. |
