...
//!
//! ```text
//! cargo run --release -p forensics --bin jitter_forensics -- \
...
const VARIANTS: &[(&str, &str, Edit)] = &[
    ...
    // field `docs/history/FUNDAMENTALS.md` §7.7 names as its successor.
    (
        "no_false_beat",
...
/// (`docs/history/FUNDAMENTALS.md` §5.1, `DECISIONS.md` 223) and there is nothing to repeat.
...
///
/// `docs/history/FUNDAMENTALS.md` §II.4 convicts every listening set this repository has ever
...
/// and delivered the one thing `docs/history/FUNDAMENTALS.md` §II.4 says a listening file has
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    writeln!(
        ...
        "# JITTER.md - the instantaneous frequency of a composite partial\n\n\
         Written by `cargo run --release -p forensics --bin jitter_forensics`. Preset \
    ...
    writeln!(
        ...
        "\nThe rungs are deliberately **not** orthogonal, and reading them as if they were \
         ...
         `docs/history/FUNDAMENTALS.md` §7.7 of `horizontal_offset_hz`, `unison_sigma_scale` and \
         `unison_coupling`."
    )?;
    writeln!(
        report,
    ...
    writeln!(
        ...
        "- `14_recording_long` / `15_eigen_fitted` - the same strike held for {LONG_S} s \
         ...
         measured set. `docs/history/FUNDAMENTALS.md` §II.4 convicts every listening set this repository \
