...
//!
//! ```text
//! cargo run --release -p forensics --bin ladder_analysis -- \
...
//!    `docs/history/TUNING_REPORT.md` §3's statistic, now computed on each *rung* rather than
...
fn validate() -> Vec<String> {
    ...
    // A partial standing on a broadband pedestal: the halo `docs/history/TUNING_REPORT.md`
...
fn write_markdown(keys: &[KeyReport], validation: &[String]) -> String {
    ...
    let _ = writeln!(
        ...
        "Objective measurements over every rung of `renders/timbre-ladder/` and the recording it \
         starts from, written by `cargo run --release -p forensics --bin ladder_analysis`. Nothing here \
    ...
    let _ = writeln!(
        ...
        "Two smooth references are fitted for metric 1 because the answer depends on how much \
         ...
         which is what `docs/history/TUNING_REPORT.md` §3 used, so its column is comparable with the report's. \
...
1. **Per-partial decays — specifically the beat structure, not the rate.** Largest and most
   ...
   C4 and C6. `docs/history/TUNING_REPORT.md` §1 and the decay stage already place the rates correctly; what is
...
3. **The attack residual — the most reliably-signed ingredient in the ladder, and not diagnostic of
   ...
   anywhere in the ladder. `docs/history/TUNING_REPORT.md` §4 refuted a missing attack *level*; this is a missing
...
* **The late treble.** At C6 the resynthesis has no measurable envelope at all at partials 5 and 6
  where the recording has one — `docs/history/TUNING_REPORT.md` §4's halo, which per-partial trajectories do not
