...
//!
//! `docs/history/TUNING_REPORT.md` measures the frequencies and the decays and finds them
...
//!
//! ```text
//! cargo run --release -p forensics --bin timbre_ladder -- \
...
/// `docs/history/TUNING_REPORT.md` §4 says the engine is already right and the complaint is
...
struct KeyReport {
    ...
    /// `docs/history/TUNING_REPORT.md` §3's number, recomputed here because it is the size
...
/// `docs/history/TUNING_REPORT.md` §1 says C6 has six), and otherwise the comb the *preset*
...
/// rather than at the strike: `docs/history/TUNING_REPORT.md` §5 says the balance *drifts*
...
/// RMS of a set of ratios, in dB — `docs/history/TUNING_REPORT.md` §3's statistic.
...
fn write_readme(
    ...
    text.push_str(
        "# timbre-ladder\n\n\
         ...
         `cargo run --release -p forensics --bin timbre_ladder`; one directory per key, ten files each, \
    ...
    text.push_str(&format!(
        "\nSource: the Salamander Grand Piano library at velocity {VELOCITY} (nearest layer), \
         ...
         reproduce the 1-6 dB of balance *drift* `docs/history/TUNING_REPORT.md` \u{a7}5 measures.\n\
         - **The additive rungs are only the tracked partials.** Where the recording's late \
         energy is not the struck string - `docs/history/TUNING_REPORT.md` \u{a7}4 puts C6's between-partial \
