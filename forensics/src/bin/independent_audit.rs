...
//!
//! ```text
//! cargo run --release -p forensics --bin independent_audit -- [before.toml] [1 2 ...]
//! ```
...
/// Power between the partials over power in them, dB — `docs/history/TUNING_REPORT.md` §4's
...
/// were.
///
/// `docs/history/TUNING_REPORT.md` §5 quotes `harmLC3` at −30.7 dB and `harmLC5` at
...
/// the "rings 1–2 s" half of `docs/history/TUNING_REPORT.md` §5's target, measured the same
...
///
/// `docs/history/TUNING_REPORT.md`'s backlog item 5 says of this gap that "the level is a
