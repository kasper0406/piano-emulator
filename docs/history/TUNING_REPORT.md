...
> **Status: investigation record from 2026-08-13** (last revised 2026-08-14),
> kept for the measurements in it and for the reasoning behind the decisions it
> caused. **It is not a description of the instrument as it stands.**
> Its conclusions were acted on in `DECISIONS.md` **98–133** (the fields Phase E
> added: `inharmonicity_b4`, the per-string decay spread, the polarization pan
> spread, the strike comb's contact taper, `[noise]`), **145–207** (the
> sympathetic milestone, the duplex segments, the per-partial fields and their
> coverage) and **237** (`notes.partial_gains` became the full measured ratio).
> **Superseded where noted**, and in three places wholesale: §6's per-string
> decay spread is an *output* of the coupled unison rather than a field
> (`DECISIONS.md` 225, `docs/history/FUNDAMENTALS.md` §5); the treble aftersound
> gap of §4 is on the soundboard's late field and not on the sympathetic
> coupling (`DECISIONS.md` 184); and its envelope-residual metric is reported by
> its own control as unusable. Where this file and `DECISIONS.md` disagree, the
> log wins. Live documents: `README.md`, `TUNING.md`, `PHYSICS.md`, `SPEC.md`.
>
> The `residuals` subcommand that produced it is still re-runnable, and the
> command below is current.
...
measurement taken by `tuner/src/tools/residuals.rs` over the 450 analysed recordings and over the
engine's own renders of the same notes; nothing is estimated by eye or taken from the literature.

```
cargo run --release -p piano-tuner -- residuals    # ~2.5 min against the warm cache
```
...
`tuner/src/tools/residuals.rs` (the driver). Both read the same cached trajectories the survey used,
