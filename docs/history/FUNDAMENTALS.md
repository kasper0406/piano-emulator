...
> **Status: investigation record from 2026-08-14** (last revised 2026-08-15),
> kept because it is the derivation behind the coupled string model and the
> argument that convicted the one it replaced. **It reviews a string model the
> engine no longer has**: §§0–4 describe `engine/src/string.rs` *before* the
> unison became a `2N × 2N` eigenproblem, and every verdict marked **WRONG**
> there was acted on rather than left standing.
> Conclusions acted on in `DECISIONS.md` **223–261** — 223 built the
> eigenproblem, 225 made `voicing.unison_coupling`,
> `voicing.horizontal_offset_hz` and `voicing.unison_sigma_scale` inert
> (**324** has since stripped all three from the shipped presets, so the values
> quoted in §§2.2, 3.4 and 6 are no longer in any file), 226 inverted
> `horizontal_decay_ratio`, 230 measured the metronome gone, 233 and 239 built
> `notes.false_beat` as the successor mechanism, and 235 answered §7.3 with
> `[voicing.strike_direction]` — and the perception half's complaint became a
> standing gate in **296–302** (`tuner/tests/melody.rs`).
> **Superseded where noted.** §7.7's table of "what no preset table can reach"
> is the part that still reads forward; the rest is history. Where this file and
> `DECISIONS.md` disagree, the log wins. Live documents: `README.md`,
> `TUNING.md`, `PHYSICS.md`, `SPEC.md`.
...
| `voicing.unison_sigma_scale` | **largely redundant** | the per-string decay split it was built for (`DECISIONS.md` 105, `docs/history/TUNING_REPORT.md` §6) now falls out of the eigenproblem |
...
`renders/realism/REALISM.md`, `docs/history/TUNING_REPORT.md` §§3, 5, 6, `PHYSICS.md` §4,
...
Both are promotions of measurements that already exist in `forensics/src/bin/jitter_forensics.rs`; both satisfy `ANALYSIS.md` §7's own certification rule (far for the engine, near-zero for an exact resynthesis: rung 01 plays the recording's *measured* f_k(t) and a_k(t), so it inherits the recording's jitter and beat structure by construction, while the forensics' controls put the metric floor at 0.00–0.05 c). Both should be validated exactly as §7 validates: reference-vs-neighbouring-velocity-layer is the floor, rung 01-vs-00 must read inside it.
