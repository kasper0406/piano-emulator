...
//!     --example chain_fit -- [data/salamander] [renders/chain] [preset.toml]
//! ```
...
fn render_engine_phrase(preset: &Preset, phrase: &Phrase) -> Audio {
    let (left, right) =
        render_to_buffer(preset, &to_engine_events(&phrase.events), phrase.duration_s as f32);
...
Written by `cargo run --release -p piano-tuner --features diagnostics --example chain_fit`. \
