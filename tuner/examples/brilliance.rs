...
//!     --example brilliance -- data/salamander presets/salamander-c5.toml
//! cargo run --release -p piano-tuner --features diagnostics \
//!     --example brilliance -- data/salamander presets/salamander-c5.toml 0
//! cargo run --release -p piano-tuner --features diagnostics \
//!     --example brilliance -- data/salamander presets/salamander-c5.toml --trim 6
//! ```
...
/// Seconds of note, matching `compass_scan` so the reference cache is shared.
...
/// The recording of one note, onset-aligned exactly the way `compass_scan`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Byte-for-byte the key `compass_scan` uses, so its warm cache is this
