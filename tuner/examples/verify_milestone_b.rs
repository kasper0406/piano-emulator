...
//! rendered audio with the same code `TUNING_REPORT.md` §4 measured the
...
//!
//! ```text
//! cargo run --release --example verify_milestone_b -- [before.toml] [1 2 3 4 5]
//! ```
//!
...
/// What one census frame came to, in the columns `TUNING_REPORT.md` §4 uses.
...
/// `TUNING_REPORT.md` §4 mean the same thing.
fn census_one(
    signal: &[f32],
    key: u8,
    preset: &Preset,
...
/// `TUNING_REPORT.md` §4 and §5's five numbers, measured on the presets as
/// they sit on disk.
///
/// The gestures are `fit_sympathetic.rs`'s, because a verification that used a
...
/// `fit_sympathetic.rs`'s `halo_level`, term for term: the sympathetic
