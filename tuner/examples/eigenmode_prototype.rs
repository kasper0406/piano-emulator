...
//!
//! ```text
//! cargo run --release --example eigenmode_prototype -- \
//!     [data/salamander] [presets/salamander-c5.toml] [renders/jitter]
//! ```
//!
//! # What this is
//!
//! `FUNDAMENTALS.md` §5 derives a replacement for `engine/src/string.rs`'s
...
//! second order — `FUNDAMENTALS.md` §2.5, deliberately deferred), and each block
...
//!
//! 1. **`radiated_share` against `horizontal_decay_ratio`** — `FUNDAMENTALS.md`
...
/// second is past every crossing time `FUNDAMENTALS.md` §2.4 tabulates.
...
/// `voicing.horizontal_decay_ratio` — `FUNDAMENTALS.md` §5.4's re-reading of the
...
/// coupling constant of `FUNDAMENTALS.md` §1.1, derived rather than read.
///
...
/// non-normal matrix (`FUNDAMENTALS.md` §5.1).
fn block_modes(
    omegas: &[f64],
    sigma_int: f64,
    c: Complex64,
...
/// `engine::modal::ModalBank` runs, with the one difference `FUNDAMENTALS.md`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    writeln!(
        ...
        "# EIGENMODE.md - the coupled-eigenmode unison, prototyped and measured\n\n\
         Written by `cargo run --release --example eigenmode_prototype`. Preset \
         `{preset_path}`, velocity {VELOCITY} unless a row says otherwise. `engine/` is not \
         touched: `03_eigenmode` is an offline modal renderer with `FUNDAMENTALS.md` §5's \
    ...
    for (key, name) in KEYS {
        ...
        writeln!(
            ...
            "\n**{name} - velocity spread.** The range of each cell over velocities \
             ...
             the spread of the beat depth. `FUNDAMENTALS.md` §4.1 property 4 says a \
    ...
    writeln!(
        ...
        "\n## The listening set\n\n\
         ...
         - `03_eigenmode` - the same renderer with `FUNDAMENTALS.md` §5's construction\n\
