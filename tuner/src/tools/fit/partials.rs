...
//!    (`estimate::shaping`; `docs/history/TUNING_REPORT.md` §3, `ANALYSIS.md` §4a).
...
//!    decay stage's own gates pass (`estimate::shaping`; `docs/history/TUNING_REPORT.md` §2).
...
//!
//! **`notes.partial_gains` has moved.** The motion stages ([`super::motion`]) fit it as
...
//! were derived together; run the motion stages afterwards and their table replaces
//! this one outright.
//!
//! ```sh
//! cargo run --release -p piano-tuner -- fit --stage partials \
...
/// Velocity every engine reference gesture is played at — `docs/history/TUNING_REPORT.md`
...
fn warn_if_already_fitted(base: &Preset) {
    ...
    eprintln!(
        "fit --stage partials: warning — the base preset already carries {}. This tool is \
