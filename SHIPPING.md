...
- **Data licensing:** every preset's source library is recorded in
  `ATTRIBUTION.md` before its parameters ship. Salamander is CC-BY; MAESTRO
  (unused so far) is CC BY-NC-SA, acceptable for a free product if ever used.
...
- **New measured piano** = a sample library + a library adapter (the survey and
  the reference sampler currently assume Salamander's SFZ shape: minor-third
  layout, 16 layers, 48 kHz — chromatic or sparser layouts need the adapter
  layer generalized) + one factory run + listening QA. License recorded first.
